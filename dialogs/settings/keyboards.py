...
#     Button(Const("🌐 Язык интерфейса"), id="search_menu", on_click=language_menu_handler, when=always_false),
# ]
...
def languages_buttons():
    return Column(
        ...
        Checkbox(Const("✓ 🇪🇸 Español"), Const("🇪🇸 Español"),
                 id='spain', on_state_changed=language_changed, when=always_false),
        Checkbox(Const("✓ 🇫🇷 Français"), Const("🇫🇷 Français"),
                 id='french', on_state_changed=language_changed, when=always_false),
    )
