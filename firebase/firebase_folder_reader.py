...
async def get_folders_in_folder(folder_id=ROOT_FOLDER_ID):
...
async def get_folders_count_in_folder(folder_id=ROOT_FOLDER_ID):
    folders_in_folder = await get_folders_in_folder(folder_id)
    return len(folders_in_folder)


async def get_user_folders_deep_count(folder_id=ROOT_FOLDER_ID):
    folders_in_folder = await get_folders_in_folder(folder_id)
    deep_count = len(folders_in_folder)
    for sub_folder_id in folders_in_folder:
        deep_count += await get_user_folders_deep_count(sub_folder_id)
    return deep_count
